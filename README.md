...
After rewriting the keyword arm to OR semantics (and pinning the two failing cases as regression tests, written RED before the fix), the run scores 14/15.

That fix had a second-order cost I only found later, when a refusal in the running app sent me back to the scores. An OR arm matches 232 of the 376 chunks — 62% of the corpus — and ranks them by how many commodity lexemes they contain, which is close to no signal. Plain RRF gives that ordering the same authority as the vector arm's, and the arithmetic does the rest: at RRF_K=60 over a pool of 24 the whole rank spread is 27%, while being in both arms is worth up to 100%. Co-occurrence outweighed every rank difference by about 4×, so noise co-hits evicted the vector arm's own top hits — on the failing question, the window held nothing from the only paper that answers it while vector ranks 1, 2 and 3 were all from that paper. The vector arm now keeps half the window and the arms compete for the rest (decision 22). Two silent refusal paths now log which half failed, which is what made the report take an afternoon instead of a minute (decision 23).

The remaining eval failure is a documented recall limit. I had recorded it as the answering chunk sitting at vec_rank=120; that is the chunk carrying the sinusoid formula. The chunk that actually answers the question is the prose of section 3.5, at vec_rank=25 — one place outside the pool of 24. Widening the pool does not help: it stays rank 25. I handed the model the vector-only top 6 with all three Attention chunks in it and it still refused, so the conclusion I first wrote — a job for a reranker, not for Boolean semantics — survives, but the evidence behind it did not, and a chunk boundary splitting section 3.5 makes semantic chunking a candidate too (decision 24). As a side finding, the evals also proved that the rank-based score floor is a no-op by construction, which is now handled openly in the decision log rather than left as theory.
